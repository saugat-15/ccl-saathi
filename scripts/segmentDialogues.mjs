 ...
 *
 * SKIP_IF_EXISTS: when set, if reference.json and at least one segment .mp3 exist under OUTPUT_DIR,
 * skips PDF/GPT/Whisper/ffmpeg and still uploads full.mp3, segments, and reference.json to S3.
 *
...
const assertCanReadDialoguesMaterials = (dialoguesToCheck) => {
  ...
  const samplePdf = path.join(
    DIALOGUES_DIR,
    `${dialoguesToCheck[0].name}.pdf`,
  );
  if (!fs.existsSync(samplePdf)) {
    console.warn(
      `  ⚠️  No sample PDF at ${path.basename(samplePdf)} — skipping folder read check`,
    );
    return;
  }
...
const validatePdfExtraction = (data) => {
  ...
  for (const seg of byIndex) {
    if (typeof seg.segmentIndex !== "number") {
      throw new Error(
        `Invalid segment: segmentIndex must be a number (got ${JSON.stringify(seg).slice(0, 160)})`,
      );
    }
    if (seen.has(seg.segmentIndex)) {
      throw new Error(`Duplicate segmentIndex ${seg.segmentIndex} in GPT output`);
    }
...
${pdfText}`,
  ...
  try {
    ...
    if (parseErr instanceof SyntaxError) {
      ...
      throw new Error("GPT did not return valid JSON (try again; API may have truncated)");
    }
    throw parseErr;
  }
};
...
const countWhisperIntroSegmentsToSkip = (whisperSegments) => {
  const pieces = whisperSegments.map((s) => {
    const t =
      s != null && typeof s === "object" && "text" in s && typeof s.text === "string"
        ? s.text
        : "";
    return t.trim();
  });
...
const detectSilencesWithFfmpeg = (
  mp3Path,
  noiseDb,
  minDurationSec,
) => {
 ...
 */
const speechRegionsFromSilences = (
  silences,
...
const trimSpeechRegionsFromDialogueStart = (
  regions,
  dialogueStartSec,
) => {
  /** @type {SpeechRegion[]} */
...
const buildMappedTimestamps = (
  ...
  const compositeSilences = mergeNearbySilenceIntervals(silences, SILENCE_MERGE_GAP_SEC);
  console.log(
    `  🔕 Merged into ${compositeSilences.length} composite boundaries (gap ≤ ${SILENCE_MERGE_GAP_SEC}s merges chime + interpretation window)`,
  );

  const rawRegions = speechRegionsFromSilences(compositeSilences, audioDurationSeconds);
  const trimmed = trimSpeechRegionsFromDialogueStart(rawRegions, dialogueStartSec);
  ...
  if (candidates.length === pdfSegments.length + 1) {
    ...
    console.log(
      `  📋 One extra speech region at end (${outro.start.toFixed(2)}s–${outro.end.toFixed(2)}s, ${(outro.end - outro.start).toFixed(2)}s) — treating as outro, dropping`,
    );
  ...
  console.log(
    `  🔔 Cuts from silence/chime gaps: ${merged.length} segments`,
  );
...
const processDialogue = async (dialogue) => {
  ...
  const dialogueOut = path.join(OUTPUT_DIR, dialogue.id);
